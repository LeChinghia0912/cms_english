...
const UpdateQuestion = ({ lesson, question }) => {
  const { data: auth } = useSession();
  const access_token = get(auth, ["tokens", "access_token"]);

  // State quản lý đáp án
  const [options, setOptions] = useState(question?.options); // Mảng chứa các đáp án
  const [layer, setLayer] = useState(question?.layer);
  const [mutiSelect, setMutiSelect] = useState(question?.mutiSelect);
  ...
  const onSubmit = useCallback(
    ...
    [lesson, options, layer, mutiSelect, question]
  );

  return (
    <Drawer
      labelOpen="update-question"
      heading="Tạo Mới"
    ...
    >
      <div className="flex flex-col gap-4 px-4">
        ...
        <Button
          title="Chỉnh sửa"
          buttonClassName="bg-blue-500 hover:bg-blue-600 text-white hover:opacity-90"
